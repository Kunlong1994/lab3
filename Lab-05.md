...
The [second example](https://github.com/FAR-Lab/Developing-and-Designing-Interactive-Devices/blob/docs/uselessBox_loop_lab5.ino) sets an interrupt on the input pin the switch is attached to, letting the processor call a registered interrupt service function when it changes.  In this example the main loop is not required to do anything at all, though we use it to output some state information to the Serial console for fun. 
